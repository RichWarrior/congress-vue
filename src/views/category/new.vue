...
import {GET_MAIN_CATEGORIES,NEW_CATEGORY} from '@/store/action.type';
const initialize = () => {
  return Object.assign({}, categoryEntity);
};
export default {
  data: () => ({
    formValid: false,
  ...
  methods: {
    ...
    saveItem(){
        if(this.formValid){
            this.$store.dispatch(NEW_CATEGORY,this.category).then((response)=>{
